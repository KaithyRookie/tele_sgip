 ...
 * 希望大家marker的博客: http://www.yl-blog.com
 * kaithy的博客：https://kaithyrookie.github.io/
 * */

public class Test {
	 
	public static void testMT() throws Exception{
		Connection conn = new Connection("202.75.248.205", 9002);
	 
        Bind bind = new Bind(1,"sijingc","sijingc");
		...
		//本地用户帐号密码（网关访问本地参数）
        session.setLocalUser("sijingc");
        session.setLocalPass("sijingc");
        session.setLocalPort(9002);
    	...
    	String SPNumber     = "68608689";//之后为附加码
		String ChargeNumber = "68608689";
		String[] UserNumber = "8615722922386".split(",");//拆分手机号码
		String CorpId       = "49107";
		...
		try {
			MessageContent = "这是一封测试短信!".getBytes("GBK");
		...
		String reserve = "0";
		//通讯节点
		long nodeid = (3000000000L + Long.parseLong("1121171730") * 100000L + Long.parseLong(CorpId));
